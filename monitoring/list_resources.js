...
var ListResources = {
  ...
  listTimeseries: function(authClient, projectId, callback) {
    ...
    monitoring.projects.timeSeries.list({
      ...
      if (error) {
        console.error('Error retrieving timeseries');
        return;
      }
      callback(timeSeries);
    });
...
function main() {
  ...
  ListResources.getMonitoringClient(function(authClient) {
    // Create the service object.
    async.series([
      function(callback) {
        ListResources.listMonitoredResourceDescriptors(authClient, PROJECT_ID,
          function(monitoredResources) {
            console.log('listMonitoredResources(): ');
            console.log(monitoredResources);
            callback();
          });
      }, function(callback) {
        ListResources.listMetricDescriptors(authClient, PROJECT_ID,
          function(metricDescriptors) {
            console.log('listMetricDesciptors(): ');
            console.log(metricDescriptors);
            callback();
          });
      }, function(callback) {
        ListResources.listTimeseries(authClient, PROJECT_ID,
          function(timeSeries) {
